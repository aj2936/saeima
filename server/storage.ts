...
export class DbStorage implements IStorage {
  ...
  private async initializeDeputies() {
    // Check if deputies exist in the database
    const existingDeputies = await db.select().from(deputies);
    
    if (existingDeputies.length === 0) {
      // If no deputies exist, insert them
      const deputyData = [
      ...
      for (const deputy of deputyData) {
        await db.insert(deputies).values(deputy);
      }
    }
  }
  ...
  async getDeputies(): Promise<Deputy[]> {
    return db.select().from(deputies);
  }
  ...
  async voteForDeputy(userId: number, deputyId: string): Promise<boolean> {
    ...
    // Check if already has 5 votes or has already voted for this deputy
    ...
    // Update deputy votes count
    await db
      .update(deputies)
      .set({ votes: deputyResult[0].votes + 1 })
      .where(eq(deputies.id, deputyId));
